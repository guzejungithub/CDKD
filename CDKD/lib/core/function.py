...
def train_simdr(config, train_loader, model, criterion, optimizer, epoch,
              final_output_dir, tb_log_dir, writer_dict, teach_model, LR_manager,HR_manager, criterion_distill , mlp_net, decay_value):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    ...
    for i, (input1,input2,target, target_weight, meta) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        ...
        # compute output
        with torch.no_grad():
            HR_output_x, HR_output_y =  teach_model(input1)

        # compute output
        output_x, output_y = model(input2)



        temp = mlp_net(output_x,output_y,HR_output_x,HR_output_y,decay_value)  # (teacher_output, student_output)
        temp = 1.0 + 20.0 * torch.sigmoid(temp)
        temp = temp.cuda()

        target = target.cuda(non_blocking=True).long()
        target_weight = target_weight.cuda(non_blocking=True).float()

        loss_line,loss_adptive = criterion_distill(LR_manager.feature, HR_manager.feature,temp)
        loss_simcc = criterion(output_x, output_y, target, target_weight) * 1000.0
        # loss_attn_distill = 0
        loss_line = config.LOSS.ALPHA * loss_line
        loss_adptive = config.LOSS.BETA * loss_adptive
        loss = loss_simcc +loss_line + loss_adptive
        ...
        if i % config.PRINT_FREQ == 0:
            msg = 'Epoch: [{0}][{1}/{2}]\t' \
                  'Time {batch_time.val:.3f}s ({batch_time.avg:.3f}s)\t' \
                  'loss_line {loss_line:.8f} \t' \
                  'loss_adptive {loss_adptive:.8f} \t' \
                  'loss_simcc {loss_simcc:.8f} \t' \
                  'Speed {speed:.1f} samples/s\t' \
                  'Data {data_time.val:.3f}s ({data_time.avg:.3f}s)\t' \
                  'Loss {loss.val:.5f} ({loss.avg:.5f})\t'.format(
                      epoch, i, len(train_loader), batch_time=batch_time,loss_line=loss_line,loss_adptive=loss_adptive,loss_simcc=loss_simcc,
                      speed=input1.size(0)/batch_time.val,
                      data_time=data_time, loss=losses)
            logger.info(msg)
...
def validate_simdr(config, val_loader, val_dataset, model, criterion, output_dir,
    ...
    with torch.no_grad():
        ...
        for i, (input1,input2, target, target_weight, meta) in enumerate(val_loader):
            # compute output
            input = input2
