...
from utils.hook import attention_manager,attention_manager_teacher
from utils.distill_loss import DistillLoss
...
def parse_args():
    ...
    parser.add_argument('--prevModelDir',
                        ...
                        type=str,
                        default='')
    parser.add_argument('--distill_attn_param',
                        help='distill_attn_param',
                        type=float,
                        default=0.400) 
...
def main():
    ...
    teach_model = eval('models.'+'pose_hrnet_teacher'+'.get_pose_net')(
        cfg, is_train=False
    )
    ...
    teach_model.load_state_dict(torch.load(cfg.TEACHER_PATH), strict=False)
    teach_model = torch.nn.DataParallel(teach_model, device_ids=cfg.GPUS).cuda()
    
    teach_model.eval()


    criterion_distill = DistillLoss(( cfg.MODEL.IMAGE_SIZE)[0],cfg.MODEL.NUM_JOINTS, distill_attn_param=args.distill_attn_param ).cuda()
    ...
    LR_manager = attention_manager(model)
    HR_manager = attention_manager_teacher(teach_model)

    gradient_decay = CosineDecay(max_value=1.0, min_value=0.0, num_loops= 210)
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        if cfg.MODEL.COORD_REPRESENTATION == 'simdr':
            ...
            train_simdr(cfg, train_loader, model, criterion, optimizer, epoch,
              final_output_dir, tb_log_dir, writer_dict, teach_model, LR_manager,HR_manager, criterion_distill , mlp, decay_value)
